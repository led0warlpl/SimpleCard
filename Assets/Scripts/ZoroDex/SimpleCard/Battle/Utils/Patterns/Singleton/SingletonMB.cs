...
{
    /// <summary>
    ///     Singleton MonoBehaviour Implementation. Refs below:
    ...
    public class SingletonMB<T> : MonoBehaviour where T : class
    {
        ...
        void HandleDuplication()
        {
            ...
            {
                //if not silent, we raise an error with the names of the all the objects
                var singletonsNames = string.Empty;
                foreach (var duplicated in allSingletonsOfThis)
                    singletonsNames += duplicated.name + ", ";
                
                //throws an error with all objects that have this monobehaviour as message
