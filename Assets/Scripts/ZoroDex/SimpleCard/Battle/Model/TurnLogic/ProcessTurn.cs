...
namespace ZoroDex.SimpleCard.Battle
{
    ...
    public class ProcessTurn : ITurnLogic
    {
        ...
        public ProcessTurn(List<IPlayer> players,
            PlayerSeat start = PlayerSeat.Left,
            PlayerSeat current = PlayerSeat.Left)
        {
            if (players == null)
                throw new ArgumentException("A Null List is not valid argument to Create a TurnLogic");
            if (players.Count < 1)
                throw new ArgumentException("Invailid number of players: " + players.Count);
